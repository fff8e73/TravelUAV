"""
TravelUAV HTTP 服务端 - 兼容 VLA_Habitat 客户端接口

提供 HTTP 接口供 VLA_Habitat 的 HTTPTrajectoryClient 调用
接口格式与 server_Gr00t.py 保持一致
"""
...
class TravelUAVHTTPServer:
    ...
    接收来自 VLA_Habitat 客户端的请求，使用 TravelUAV 模型生成动作
    """
    ...
    def _setup_routes(self):
        ...
        async def act(request: Dict):
            """
            接收 VLA_Habitat 客户端的请求，生成动作

            请求格式:
            {
                "observation": {
                    "rgb": <numpy.ndarray>,      # (H, W, 3) RGB image
                    "depth": <numpy.ndarray>,    # (H, W) depth image
    ...
    def _process_observation(self, obs: Dict) -> List[int]:
        ...
        # 构建模拟的 episode 数据
        # TravelUAV 的模型期望 episode 格式
        episode = [{
            'instruction': instruction,
            'rgb': [image],
            'sensors': {
                'imu': {
                    'rotation': [[1, 0, 0], [0, 1, 0], [0, 0, 1]]  # 简化的旋转矩阵
                },
                'state': {
        ...
        # 目标位置（从 GPS 和 yaw 推断）
        # 这里使用一个简单的假设：目标在当前朝向的前方
        # 实际应用中，客户端应该提供目标位置
        target_distance = 10.0  # 假设目标在前方 10 米
